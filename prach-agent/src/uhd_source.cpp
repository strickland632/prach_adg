...
//  uuagent_error_e RF_ZMQ::collect_iq_data(const all_args_t& args) //{
 ...
 try {
   ...
   while (total_received < nof_samples) {
    ...
    //  if (md.error_code != uhd::rxdevice_args_metadata_t::ERROR_CODE_NONE) {
    //    res.msg = md.strerror();
    //    res.type = SOURCE_UHD_ERROR;
    //    return res;
    //  }
...
// Transmit (prach)) buffer
// The wire format is sc16, so UHD will convert
// source_error_t send(cf_t_1_1* buffer, size_t nof_samples)
...
//                             )
{
 ...
 //result object to return success or error info.
 source_error_t res = {};
 ...
 try {
  ...
  //  usrp->set_tx_gain(tx_gain_db);
   ...
   // Determine the maximum number of samples per packet(MTU).
